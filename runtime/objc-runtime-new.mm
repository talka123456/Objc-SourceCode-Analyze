...
* Like lookUpImpOrForward, but does not search superclasses. 类似lookUpImpOrForward函数， 但是不查找 superclass
...
IMP lookupMethodInClassAndLoadCache(Class cls, SEL sel)
{
    ...
    // 这个是不完整的，没有解析器，initialize 初始化等
    // 但是只是用于  .cxx_construct/destruct 两个函数， 所以其他类型的 sel 都是抛出断言
    ASSERT(sel == SEL_cxx_construct  ||  sel == SEL_cxx_destruct);

    // Search cache first.
    //
    // If the cache used for the lookup is preoptimized, 如果用于查找的缓存是预先优化的
    // we ask for `_objc_msgForward_impcache` to be returned on cache misses, 则缓存没找到则返回 img = _objc_msgForward_impcache, 。_objc_msgForward_impcache是一个标记，这个标记用来表示在父类的缓存中停止继续查找。
    ...
    if (slowpath(imp == nil)) {
        // Cache miss. Search method list. 缓存没找到， 查找函数列表

        // runtime 上锁
        mutex_locker_t lock(runtimeLock);

        // 不查找父类， 查找到则缓存到 cache 否则 imp 赋值为_objc_msgForward_impcache
        ...
        // isConstantOptimizedCache 内部严格模式下， 判断地址第一位是否为 1
...
void *objc_destructInstance(id obj) 
{
    if (obj) {
        ...
        // 销毁顺序很重要
        // 销毁析构函数
        // // 对象拥有成员变量时编译器会自动插入.cxx_desctruct方法用于自动释放,可打印方法名证明; 所以该函数是执行变量释放的工作
