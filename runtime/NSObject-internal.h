...
{
    // 64 位 系统上 SUPPORT_AUTORELEASEPOOL_DEDUP_PTRS = 1
#if SUPPORT_AUTORELEASEPOOL_DEDUP_PTRS
    // 可以理解为 Autorelease 为了关联对象创建的实体类， 只有两个相邻的 release obj 是同一个时才使用，
    struct AutoreleasePoolEntry {
        uintptr_t ptr: 48; // 关联的被 push 的对象
        uintptr_t count: 16; // 关联对象已经 push 的次数
	...
	magic_t const magic; // 对当前AutoreleasePoolPage 完整性的校验 16 字节
	__unsafe_unretained id *next; // 指向下一个即将产生的autoreleased对象的存放位置（当next == begin()时，表示AutoreleasePoolPage为空；当next == end()时，表示AutoreleasePoolPage已满 8 字节
	pthread_t const thread; // 当前线程，表明autorelease与线程有对应关系 8 字节
	AutoreleasePoolPage * const parent; // 上一页 8 字节
	AutoreleasePoolPage *child; // 下一页 8 字节
	uint32_t const depth; // 深度,第一个page的depth为0，往后每递增一个page，depth会加1； 4 字节
	uint32_t hiwat; // 表示high water mark（最高水位标记）,记录最大入栈数量标记 4 字节
