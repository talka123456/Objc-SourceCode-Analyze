...
static void object_cxxDestructFromClass(id obj, Class cls)
{
    ...
    for ( ; cls; cls = cls->getSuperclass()) {
        ...
        // 查找析构函数，lookupMethodInClassAndLoadCache不对 superclass 进行查询
        // SEL_cxx_destruct 是在 map_images 时在 Sel_init 中赋值的， 其实就是 .cxx_destruct 函数
        dtor = (void(*)(id))
            lookupMethodInClassAndLoadCache(cls, SEL_cxx_destruct);
        
        //todo:hw 如果 dtor 不等于_objc_msgForward_impcache， _objc_msgForward_impcache和消息流程有关， 后续学到再解释
