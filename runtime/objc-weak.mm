 ...
 * 返回给定引用的弱引用表中的实体数据， 没有实体数据的返回 NULL
 */
static weak_entry_t *
weak_entry_for_referent(weak_table_t *weak_table, objc_object *referent)
{
    ...
    // weak_table 中所有的 weak_entry_t 类型的实体
    weak_entry_t *weak_entries = weak_table->weak_entries;

    // 不存在返回 nil
    if (!weak_entries) return nil;
    
    // 用引用指针的哈希值 & 弱引用表中的 mask 掩码
    size_t begin = hash_pointer(referent) & weak_table->mask;
    size_t index = begin;
    size_t hash_displacement = 0;
    // 遍历weak_entries中的所有 weak_entry_t 对象， 如果不等于给定引用， 则 index+1
    while (weak_table->weak_entries[index].referent != referent) {
        index = (index+1) & weak_table->mask;
        // 如果 index == begin 表示是个坏的弱引用表
...
/**
 * 被 dealloc 调用， 不需要加锁， 淘汰所有指向提供的对象的弱指针，以便不再使用它们
