...
#   define POOL_BOUNDARY nil
    ...
    busted_die() const
    {
        // 抛出异常
        busted(_objc_fatal);
        // _builtin_unreachable 函数用于通知编译器在CPU运行时期间永远不会到达此行，
        __builtin_unreachable();
    }

    // 校验 page是否完整， 使用 magic 和线程, 默认入参==true,
    inline void
    ...
    // 对 end 的定义是 autorelease 对象 + 大小 SIZE被赋值为 1<<14 = 4K
    id * end() {
...
// 64位系统为 1
#if SUPPORT_AUTORELEASEPOOL_DEDUP_PTRS
        // 两个环境变量 任意一个位 false ； DisableAutoreleaseCoalescing == false 表示开启合并， & DisableAutoreleaseCoalescingLRU使用 LRU
        if (!DisableAutoreleaseCoalescing || !DisableAutoreleaseCoalescingLRU) {
            if (!DisableAutoreleaseCoalescingLRU) {
                ...
                if (!empty() && (obj != POOL_BOUNDARY)) {
                    ...
                    for (uintptr_t offset = 0; offset < 4; offset++) {
                        ...
                        if (offsetEntry->ptr == (uintptr_t)obj && offsetEntry->count < AutoreleasePoolEntry::maxCount) {
                            // LRU算法实现， 将匹配到的 Entry 对象偏移的 Entry (offset + 1)， 统一向上位移一位， 然后将匹配到的 Entry 填充到 top,实现最新使用的保持在next-1的位置
                            if (offset > 0) {
                ...
                // DisableAutoreleaseCoalescing == false 表示开启合并，但是只会向上查一个
                // 如果不为空， 且push对象不是哨兵对象
                if (!empty() && (obj != POOL_BOUNDARY)) {
                    // 获取 (next - 1)上一个实体
                    AutoreleasePoolEntry *prevEntry = (AutoreleasePoolEntry *)next - 1;
                    // 如果上一个实体指向的对象和当前 push 的是同一个， 则不存储地址指针， 只计数++
...
#endif
        // 返回值，赋值调用返回比直接 next-1快
        ret = next;  // faster than `return next-1` because of aliasing
        // 将 autorelease 添加的对象存储到哨兵对象的下一个位置
        *next++ = obj;
#if SUPPORT_AUTORELEASEPOOL_DEDUP_PTRS
    ...
    // 作用是标识已经创建了 pool 但是还未 push 进 obj时的状态
    ...
    // cold Page是指未使用的， 这里是遍历parent获取到最前端的 page
    ...
    {
        // 回去当前页。正在使用的标记为 hot 称为hotPage， 其他标记为 dirty
        AutoreleasePoolPage *page = hotPage();
        if (page && !page->full()) {
            // 未满页，
            return page->add(obj);
        } else if (page) {
            // 满页逻辑， 遍历 child, 找未使用的 page， 否则创建新的页
...
public:
    ...
    static inline void *push() 
    {
        ...
        if (slowpath(DebugPoolAllocation)) {
            ...
            // 每一个 autorelease 结构对象都使用一个新的 page
            // 逻辑是一样的， 如果存在 hotpage，则调用 Fullpage 插入， 否则调用 Nopage, 所以学习一个逻辑分支即可
...
// 引用技术表中的数值减少一半， 返回结构体    struct SidetableBorrow { size_t borrowed, remaining; }; 旧版本这里只返回了borrowed
// Move some retain counts from the side table to the isa field.
// Returns the actual count subtracted, which may be less than the request.
// delta_rc借位数据， 真正想借用的计数值，不包含（deallocing 和 weak 位占用数据的大小）
...
objc_object::sidetable_subExtraRC_nolock(size_t delta_rc)
{
    ...
    // RC_HALF左移 7 位, 再左移两位 delta_rc << SIDE_TABLE_RC_SHIFT 目的是移除后两位标志位的影响，
    // 计算借位后的大小
    size_t newRefcnt = oldRefcnt - (delta_rc << SIDE_TABLE_RC_SHIFT);
    
    // 表达式为真，什么也不做，只有delta_rc  <= 0 && weak 位 & deallocing 位数据全是 0，才抛出断言，
    ASSERT(oldRefcnt > newRefcnt);  // shouldn't underflow
    // 存储回新的值， 是存在weak 位 & deallocing位影响的
    it->second = newRefcnt;
    // 返回结构为{入参，新的数值}
...
objc_object::sidetable_release(bool locked, bool performDealloc)
{
    ...
    if (!locked) table.lock();
    // 尝试获取，如果未读取到， 则设置对应 key 的值为第二个参数
...
objc_object::sidetable_clearDeallocating()
{
    ...
    RefcountMap::iterator it = table.refcnts.find(this);
    if (it != table.refcnts.end()) {
        // 非优化 isa 时 引用计数表中的数字最后一个记录的是有没有弱引用，
